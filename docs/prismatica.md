...
> _Like a prism refracts light into its components, Prismatica breaks raw data into structured views, dashboards, and live adapters — tailored to every professional workflow._
...
| **Map View**          | Renders rows on a geographic map using `lat`/`lng` fields or address geocoding. _(Advanced — optional bonus)_                                       |
...
- **Scheduled CSV Export**: generates a CSV of the view's data on a configurable schedule (daily, weekly, monthly) and sends it to a configured email address or deposits it in a configured FTP/S3 path. _(Advanced — optional)_
...
_© DevForge — Reproduction restricted. This document is intended for the jury of the professional qualification TP Développeur Web et Web Mobile._
