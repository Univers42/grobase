...
// vite-gourmand SPA (built View/dist). The browser only ever talks to THIS
// origin, so vite-gourmand's strict CSP (connect-src 'self') is satisfied and we
...
server.listen(PORT, () => console.log(`vite-gourmand SPA → ${SELF_ORIGIN}  (proxying gateway → ${KONG.origin})`));
