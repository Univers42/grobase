...
// Savanna Park Zoo SPA (built front/dist). The browser only ever talks to THIS
// origin, so the app needs no CORS on Kong and no per-port CSP. It:
//   • serves the built SPA (../front/dist) with History-API fallback to index.html;
...
// Usage: PORT=5181 KONG_URL=http://kong:8000 node serve.mjs
...
const DIST = join(HERE, '..', 'front', 'dist');
...
server.listen(PORT, () => console.log(`savanna-zoo SPA → ${SELF_ORIGIN}  (proxying gateway → ${KONG.origin})`));
