...
// hypertube SPA (built View/dist). The browser only ever talks to THIS
// origin, so hypertube's strict CSP (connect-src 'self') is satisfied and we
// never need CORS on Kong. It:
//   • serves the built SPA (../View/dist) and ../View/public (baas-config.js),
//     with History-API fallback to index.html;
//   • rewrites the served /baas-config.js `url` to '' (same-origin relative) so
//     the client's fetches go back through this proxy;
...
// Usage: PORT=5174 KONG_URL=http://kong:8000 node serve.mjs
...
const DIST = join(VIEW, 'dist');
const PUBLIC = join(VIEW, 'public');
const PORT = Number(process.env.PORT || 5174);
const SELF_ORIGIN = `http://localhost:${PORT}`;
const API_PREFIXES = ['/auth/', '/query/', '/storage/', '/realtime/', '/rest/', '/functions/', '/media/', '/search/', '/api/', '/stream/'];
const TYPES = {
...
server.listen(PORT, () => console.log(`hypertube SPA → ${SELF_ORIGIN}  (proxying gateway → ${KONG.origin})`));
