...
use data_plane_server::ratelimit::{refill_and_take, tier_max_rows, tier_rate, TenantRateLimiter};
use serde_json::json;
...
fn refill_and_take_empty_bucket_no_time_denies() {
    ...
    for (rps, burst) in [(1, 1), (10, 5), (100, 1000), (1, 0)] {
        ...
        assert!(
            !admitted,
            "empty bucket must deny (rps={rps} burst={burst})"
        );
...
fn refill_and_take_negative_elapsed_clock_skew_adds_nothing() {
    ...
    for elapsed in [-0.001, -1.0, -1e9, f64::NEG_INFINITY] {
        ...
        assert!(
            !admitted,
            "negative elapsed {elapsed} must not admit on an empty bucket"
        );
...
fn refill_and_take_table_of_cases() {
    ...
    for &(tokens, elapsed, rps, burst, exp_tokens, exp_admit) in cases {
        ...
        assert_eq!(
            got_admit, exp_admit,
            "admit for {tokens},{elapsed},{rps},{burst}"
        );
        assert!(
...
fn refill_admits_again_after_waiting() {
    ...
    assert!(
        rl.allow("t", 100, 5),
        "refilled bucket admits after the wait"
    );
}
...
fn rps_zero_is_unlimited_for_any_burst() {
    ...
    for burst in [0u32, 1, 5, 100] {
        for _ in 0..500 {
            assert!(
                rl.allow("unlimited", 0, burst),
                "rps=0 is unlimited (burst={burst})"
            );
        }
    }
...
fn tenants_are_isolated_one_exhausted_does_not_deny_another() {
    ...
    for t in &tenants {
        assert!(
            !rl.allow(t, 1, 3),
            "{t} stays exhausted (independent bucket)"
        );
    }
    assert_eq!(
        rl.tracked(),
...
fn one_tenant_spamming_keys_cannot_starve_neighbours() {
    ...
    assert_eq!(
        victim_admits, 10,
        "the victim's bucket is untouched by the attacker"
    );
}
...
fn distinct_tenant_strings_are_distinct_buckets() {
    ...
    let rl = TenantRateLimiter::new();
    let ids = [
        "a", "A", "a ", " a", "a;b", "a' OR '1", "../etc", "🔥", "a\0b",
    ];
    for id in ids {
        assert!(rl.allow(id, 1, 1), "{id:?} first request admits");
        assert!(
            !rl.allow(id, 1, 1),
            "{id:?} second request denied (own burst=1)"
        );
    }
    assert_eq!(
        rl.tracked(),
        ids.len(),
        "each distinct string is its own bucket"
    );
}
...
fn tier_rate_burst_defaults_to_twice_rps() {
    ...
    assert_eq!(
        tier_rate(Some(&json!({ "rps": 10, "burst": 1 }))),
        Some((10, 1))
    );
}
...
fn tier_max_rows_parsing() {
    ...
    assert_eq!(
        tier_max_rows(Some(&json!({ "max_rows": u64::from(u32::MAX) + 1 }))),
        None
    );
    ...
    assert_eq!(
        tier_max_rows(Some(&json!({ "max_rows": u64::from(u32::MAX) }))),
        Some(u32::MAX)
    );
}
