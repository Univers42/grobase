...
fn unsupported_capability_display() {
    let e = DataPlaneError::UnsupportedCapability {
        engine: "redis".into(),
        capability: "aggregate".into(),
    };
    let s = e.to_string();
...
fn capability_gated_display() {
    let e = DataPlaneError::CapabilityGated {
        capability: "vector".into(),
    };
...
fn mount_not_found_display() {
    let e = DataPlaneError::MountNotFound {
        mount_id: "db-99".into(),
    };
...
fn transaction_not_found_display() {
    let e = DataPlaneError::TransactionNotFound {
        tx_id: "tx-abc".into(),
    };
...
fn invalid_identifier_display() {
    let e = DataPlaneError::InvalidIdentifier {
        value: "1; DROP".into(),
    };
...
fn credential_unavailable_display() {
    let e = DataPlaneError::CredentialUnavailable {
        mount_id: "m1".into(),
    };
...
fn credential_provider_failed_display() {
    let e = DataPlaneError::CredentialProviderFailed {
        provider: "vault".into(),
        mount_id: "m1".into(),
    };
    let s = e.to_string();
...
fn invalid_request_display() {
    let e = DataPlaneError::InvalidRequest {
...
fn not_implemented_display() {
    let e = DataPlaneError::NotImplemented {
        feature: "two-phase commit".into(),
    };
...
fn assert_prefixed(
    make: impl Fn(String) -> DataPlaneError,
    get: impl Fn(&DataPlaneError) -> Option<&str>,
) {
...
fn prefix_message_prefixes_backend() {
    assert_prefixed(
        ...
        |e| match e {
            DataPlaneError::Backend { message } => Some(message),
            _ => None,
        },
    );
}
...
fn prefix_message_prefixes_conflict() {
    assert_prefixed(
        ...
        |e| match e {
            DataPlaneError::Conflict { message } => Some(message),
            _ => None,
        },
    );
}
...
fn prefix_message_prefixes_invalid_request() {
    assert_prefixed(
        ...
        |e| match e {
            DataPlaneError::InvalidRequest { message } => Some(message),
            _ => None,
        },
    );
}
...
fn prefix_message_prefixes_invalid_identifier_value() {
    assert_prefixed(
        ...
        |e| match e {
            DataPlaneError::InvalidIdentifier { value } => Some(value),
            _ => None,
        },
    );
}
...
fn prefix_message_prefixes_not_implemented_feature() {
    assert_prefixed(
        ...
        |e| match e {
            DataPlaneError::NotImplemented { feature } => Some(feature),
            _ => None,
        },
    );
}
...
fn prefix_message_empty_prefix_is_identity_text() {
    let prefixed = DataPlaneError::prefix_message(
        "",
...
fn data_plane_result_ok_and_err() {
    ...
    let err: DataPlaneResult<u32> = Err(DataPlaneError::Backend {
        message: "x".into(),
    });
    assert!(err.is_err());
}
...
fn data_plane_error_is_std_error() {
    ...
    let boxed = as_dyn(DataPlaneError::NotImplemented {
        feature: "f".into(),
    });
...
fn prefix_message_preserves_display_status_shape_for_invalid_request() {
    ...
    let p = DataPlaneError::prefix_message(
        "batch item 3: ",
    ...
    assert!(
        p.to_string().starts_with("invalid request:"),
        "{}",
        p.to_string()
    );
