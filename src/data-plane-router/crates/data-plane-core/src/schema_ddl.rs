...
/// The single integer-typed primary-key column that carries no explicit
/// default — the column a `create_table` must auto-increment so the
/// live-database write path (id-less inserts, e.g. the osionos database block)
/// gets a server-generated key instead of a not-null violation.
///
/// Returns `None` for a composite primary key, a non-integer primary key, or a
/// caller-supplied default (an explicit default is the caller's intent and
/// wins). Each relational engine renders the auto-increment in its own dialect
/// (Postgres `GENERATED BY DEFAULT AS IDENTITY`, MySQL `AUTO_INCREMENT`, SQLite
/// inline `INTEGER PRIMARY KEY`); `BY DEFAULT`/equivalent keeps explicit-id
/// inserts (seed data) working.
#[must_use]
pub fn auto_increment_pk<'a>(
    columns: &'a [DdlColumnDef],
    primary_key: &[String],
) -> Option<&'a str> {
    let [only] = primary_key else {
        return None;
    };
    columns
        .iter()
        .find(|c| &c.name == only)
        .filter(|c| c.normalized_type == NormalizedType::Integer && c.default.is_none())
        .map(|c| c.name.as_str())
}
...
mod tests {
    ...
    #[test]
    fn auto_increment_pk_fires_only_for_a_single_default_less_int_pk() {
        let int_col = |name: &str, default: Option<&str>| DdlColumnDef {
            name: name.into(),
            normalized_type: NormalizedType::Integer,
            nullable: false,
        ...
        let id = int_col("id", None);
        assert_eq!(
            auto_increment_pk(std::slice::from_ref(&id), &["id".into()]),
            Some("id")
        );
        let with_default = int_col("id", Some("0"));
        assert_eq!(
            auto_increment_pk(std::slice::from_ref(&with_default), &["id".into()]),
            None
        );
        let text = DdlColumnDef {
            name: "slug".into(),
        ...
        assert_eq!(
            auto_increment_pk(std::slice::from_ref(&text), &["slug".into()]),
            None
        );
        let composite = vec![int_col("a", None), int_col("b", None)];
        assert_eq!(
            auto_increment_pk(&composite, &["a".into(), "b".into()]),
            None
        );
    }
