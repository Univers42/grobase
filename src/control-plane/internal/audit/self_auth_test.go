...
const selfAuthToken = "audit-self-auth-token"
...
func selfRequest(signed bool) *http.Request {
	r := httptest.NewRequest(http.MethodGet, "/v1/tenants/T/audit", nil)
...
func TestTokenOrSelf_HMACRejectsForgedHeader(t *testing.T) {
	...
	if rt.tokenOrSelf(w, selfRequest(false), "T") {
		t.Fatal("flag ON: an unsigned tenant header must NOT authorize audit reads")
	}
