...
func newReqCtx(w http.ResponseWriter, r *http.Request, log *slog.Logger) *reqCtx {
	...
	return &reqCtx{
		sw: sw, r: r, log: WithTenant(log, tenantID),
...
func (rc *reqCtx) finish() {
	...
	if !strings.HasPrefix(rc.r.URL.Path, "/health") && rc.r.URL.Path != "/metrics" {
		procMetrics.observe(rc.r.Method, rc.sw.status, time.Since(rc.start))
		procMetrics.observeTenant(rc.sw.status, rc.tenantID)
	}
