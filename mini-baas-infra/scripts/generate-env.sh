...
# PostgREST connects as this dedicated NON-superuser, NOBYPASSRLS `authenticator`
# login role (created in scripts/db-bootstrap.psql) — NOT the postgres superuser,
# which bypasses RLS unconditionally. This is the credential half of the
# safe-by-construction public-REST isolation fix (migration 065).
...
# PostgREST DSN uses the non-superuser `authenticator` role, so RLS (and FORCE RLS)
# always binds the public REST/GraphQL surface. DATABASE_URL stays the superuser
# DSN for the app/control planes that legitimately need it.
...
# PostgREST connects as the non-superuser authenticator role (NOBYPASSRLS) so
# RLS binds the public REST surface. db-bootstrap creates the role with this pwd.
