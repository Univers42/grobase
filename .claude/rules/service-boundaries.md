...
- **grobase (the backend server — self-hosted; fly.io retired) owns ALL state** — the database (it manages Postgres directly; apps never touch raw
...
- **WebSocket goes browser → the grobase server directly**, never through Vercel.
...
grobase. Never put backend secrets in Vercel env or vice-versa.
