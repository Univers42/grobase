...
_entrypoints() {
  list_files | grep -iE '(^|/)(main\.(c|go|rs|py)|index\.(ts|js)|cmd/.*/main\.go)$' | sed -n 1,10p |
    sed 's/^/- /' || true
}
...
_test_frameworks() (
  ...
  if manifest package.json; then
    if have jq; then
      ...
      seen "$root/package.json" '"vitest"' vitest
      seen "$root/package.json" '"jest"' jest
      seen "$root/package.json" '@playwright/test' playwright
      seen "$root/package.json" '"cypress"' cypress
    fi
  fi
  for f in pyproject.toml requirements.txt; do
    [ -f "$root/$f" ] && {
      seen "$root/$f" '\bpytest\b' pytest
      seen "$root/$f" '\bhypothesis\b' hypothesis
      seen "$root/$f" '\bnox\b' nox
    }
  done
  ...
  if has_ext 'c|h|cc|cpp|hpp|cxx'; then
    local m
    m="$(list_files | grep -iE '\.(c|h|cc|cpp|hpp|cxx)$' | sed "s#^#$root/#" | tr '\n' '\0' |
...
build_full() {
  ...
  local e
  e="$(_entrypoints)"
  [ -n "$e" ] && echo "$e" || echo "- (none matched the usual names)"
  echo
