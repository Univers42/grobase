...
- **`claude-deal-with-the-devil/` submodule as the `.claude/` reference:** its tools, hooks, agents, skills and rules are merged into `.claude/`; consult it for upstream changes before designing config from scratch. Its risk gate, quality bar and run-safely rules are now in `.claude/rules/` (`risk.md`, `quality-bar.md`, `run-safely.md`).
