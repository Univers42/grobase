...
write_local_overrides() {
  local pepper
  if [ -f "$DATA/.pepper" ]; then
    pepper="$(cat "$DATA/.pepper")"
  else
    pepper="$(openssl rand -hex 32)"
    printf '%s' "$pepper" >"$DATA/.pepper"
  fi
  cat >.env.local <<-EOF
...
assemble_env() {
  ...
  cp .env.secrets "$SECRETS_SAVE"
  cp .env.local "$LOCAL_SAVE"
  preflight
}
...
preflight() {
  ...
  [ "${PREFLIGHT_ENFORCE:-0}" = 1 ] && {
    log "preflight-production failed and PREFLIGHT_ENFORCE=1 — refusing to boot"
    exit 1
  }
...
bring_up() {
  ...
  log "up auth tier (gotrue + deps create auth.users)"
  $DC up -d gotrue || log "gotrue tier issue"
  wait_for_auth
  if [ -f "$MIGRATED" ]; then
  ...
  $DC up -d --no-build $SERVICES ||
    {
      sleep 15
      $DC up -d --no-build $SERVICES || log "some services unhealthy — inspect via ssh"
    }
...
wait_for_registry() {
  ...
  for _ in $(seq 1 40); do
    [ "$(docker inspect -f '{{.State.Health.Status}}' mini-baas-adapter-registry-go 2>/dev/null)" = healthy ] &&
      {
        log "adapter-registry-go healthy"
        return 0
      }
    sleep 3
  done
...
provision_apps() {
  [ -f "$PROVISIONED" ] && {
    log "apps already provisioned — skipping"
    return 0
  }
  local ok=1
  for c in $CONTRACTS; do
    ...
    KONG_URL=http://127.0.0.1:8000 bash scripts/provision-contract.sh \
      "infra/config/contracts/$c.json" 2>&1 | tail -8 || {
      ok=0
      log "provision $c failed"
    }
  done
...
main() {
  ...
  maybe_reset
  cd "$REPO"
  log "clean container slate (keeping data volume)"
