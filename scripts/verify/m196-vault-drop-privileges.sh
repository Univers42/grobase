...
# **************************************************************************** #
#                                                                              #
#  m196-vault-drop-privileges.sh — with VAULT_DROP_PRIVILEGES_ENABLED on, the  #
...
writable_by_vault() {
  docker exec -u vault "$1" sh -c 'for d in /vault /vault/config /vault/policies /vault/scripts; do
    touch "${d}/.m196" 2>/dev/null && printf "%s " "${d}"
  done; true'
}
...
leg_neg() {
  local dir=/vault/data/m196-missing state log _
