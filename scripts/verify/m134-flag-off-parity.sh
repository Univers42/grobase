...
TC_IMG="m134-tc-$$:scratch"
AR_IMG="m134-ar-$$:scratch"
NET="m134net-$$"
PG="m134-pg-$$"
TC_OFF="m134-tc-off-$$"
AR_OFF="m134-ar-off-$$"
PORT_TC="${M134_PORT_TC:-18939}"
PORT_AR="${M134_PORT_AR:-18940}"
PGPW="postgres"
...
MID="$(mount_id)"
[[ -n "${MID}" ]] || fail "(2) mount not registered (line: 2 mount)"
...
emit_gate_log() { (
  ...
  log_event GATE --gate "m134=PASS" --outcome pass \
    ...
    --ref "scripts/verify/m134-flag-off-parity.sh" >/dev/null 2>&1
  exit 0
) || true; }
emit_gate_log
