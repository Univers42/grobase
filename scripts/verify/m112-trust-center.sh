...
{ docker logs "${TC_ON}" 2>&1 || true; } | grep -qi "trust center enabled" ||
  {
    ...
    fail "trust center never reported enabled (line: TC_ON enabled log)"
  }
...
JSON
chmod 0755 "${WORK}/badtrust"
chmod 0644 "${WORK}/badtrust/posture.json"
...
for _ in $(seq 1 30); do
  if [[ "$(docker inspect -f '{{.State.Running}}' "${TC_BAD}" 2>/dev/null)" != "true" ]]; then
    BAD_RUNNING="false"
    break
  fi
  sleep 0.5
...
[[ "${BAD_RUNNING}" == "false" ]] ||
  {
    docker logs "${TC_BAD}" 2>&1 | tail -20
    fail "(B2) a DISHONEST manifest did NOT stop the boot — tenant-control still running (the honesty boundary did not fire) (line: B2 still running)"
  }
BAD_EXIT="$(docker inspect -f '{{.State.ExitCode}}' "${TC_BAD}" 2>/dev/null || echo '?')"
[[ "${BAD_EXIT}" != "0" && "${BAD_EXIT}" != "?" ]] ||
  {
    docker logs "${TC_BAD}" 2>&1 | tail -20
    fail "(B2) tenant-control exited '${BAD_EXIT}' on a dishonest manifest — it must REFUSE to boot (non-zero) (line: B2 exit code)"
  }
...
{ docker logs "${TC_OFF}" 2>&1 || true; } | grep -qi "trust center disabled" ||
  {
    ...
    fail "OFF tenant-control did not report trust center disabled (flag default not OFF?) (line: TC_OFF disabled log)"
  }
