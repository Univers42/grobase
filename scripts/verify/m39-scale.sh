...
skip() {
  printf '\033[1;33m[M39] SKIP: %s\033[0m\n' "$*"
  exit 0
}
...
metric() { # $1 metric-substring → value
  local port
  port="$(docker port "${DP}" 4011/tcp 2>/dev/null | head -1 | sed 's/.*://')"
...
R_TC0="$(restarts ${TC})"
R_AR0="$(restarts ${AR})"
EV0="$(metric 'pool_events_total{service="data-plane-router",event="evicted"' || echo 0)"
EV0="${EV0:-0}"

# ── seed ────────────────────────────────────────────────────────────────────
cyan "seeding ${SCALE} tenants…"
make -C "${ROOT}" scale-seed SCALE="${SCALE}" PREFIX="${PREFIX}" >/tmp/m39-seed.txt 2>&1 || {
  tail -5 /tmp/m39-seed.txt
  red "seed failed"
  make -C "${ROOT}" scale-teardown SCALE="${SCALE}" >/dev/null 2>&1 || true
  exit 1
}
...
SEEDED="$(grep -c '"status":"created"\|"status":"exists"' "${ROOT}/artifacts/scale/tenants-${SCALE}.jsonl" 2>/dev/null || true)"
SEEDED="${SEEDED:-0}"
ERRORED="$(grep -c '"status":"error"' "${ROOT}/artifacts/scale/tenants-${SCALE}.jsonl" 2>/dev/null || true)"
ERRORED="${ERRORED:-0}"
...
fail=0
((R_TC1 > R_TC0)) && {
  red "tenant-control restarted ${R_TC0}→${R_TC1} during seed (crypto OOM regression)"
  fail=1
}
((R_AR1 > R_AR0)) && {
  red "adapter-registry restarted ${R_AR0}→${R_AR1} during seed (crypto OOM regression)"
  fail=1
}
((ERRORED > 0)) && {
  red "${ERRORED} provisions errored"
  fail=1
}

# ── multi-tenant load ────────────────────────────────────────────────────────
cyan "driving load across ${SEEDED} tenants…"
PREFIX="${PREFIX}" SCALE="${SCALE}" bash "${ROOT}/scripts/bench/multitenant.sh" >/tmp/m39-load.txt 2>&1 || {
  tail -8 /tmp/m39-load.txt
  red "multitenant load failed"
  exit 1
}
tail -3 /tmp/m39-load.txt
MT_ART="${ROOT}/artifacts/bench/multitenant-${SCALE}.json"
[[ -f "${MT_ART}" ]] || {
  red "no multitenant artifact"
  exit 1
}
...
RSS="$(rss_mib ${DP})"
RSS="${RSS:-0}"
EV1="$(metric 'pool_events_total{service="data-plane-router",event="evicted"' || echo 0)"
EV1="${EV1:-0}"
...
if [[ -f "${BASE_ART}" ]]; then
  ...
  awk -v a="${P99}" -v b="${LIMIT}" 'BEGIN{exit !(a>b)}' && {
    red "fan-out p99 ${P99}ms > ${P99_FACTOR}× baseline (${LIMIT}ms)"
    fail=1
  }
...
((ERR5XX > 0)) && {
  red "${ERR5XX} server (5xx) errors under fan-out"
  fail=1
}
((RSS > RSS_BAR)) && {
  red "data-plane RSS ${RSS}MiB > ${RSS_BAR}MiB"
  fail=1
}
