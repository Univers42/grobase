...
# **************************************************************************** #
#                                                                              #
#  m192-functions-net-jail.sh — tenant function code must not reach the        #
...
P="m192-$$"
TENANT="m192t$$"
TOKEN="m192-dummy-service-token"
T="$(mktemp -d)" || exit 1
chmod 700 "${T}"
...
# cleanup removes every m192 container and network of this run, then the temp dir.
cleanup() {
...
runtime_args() {
  ...
  RT=(--cpus 0.5 --memory 256m --tmpfs /data --add-host "m192-rebind.test:${KONG_IP}" -v "${SRC}:/app:ro"
...
leg_base() {
  ...
  for t in kong:8001 "${KONG_IP}:8001" m192-rebind.test:8001 postgres:5432 mongo:27017; do
  ...
  ok "base: $(jq '[.[] | select(. == "open")] | length' "${r}") targets open, incl. kong:8001, ${KONG_IP}:8001, m192-rebind.test:8001, postgres, mongo — jail check fails here, as it must"
}
...
live_stack
INTERNAL=(kong:8001 kong:8002 mini-baas-kong:8001 "${KONG_IP}:8001" m192-rebind.test:8001 vault:8200 postgres:5432
