...
SVC="${LIVE_SERVICE_APIKEY}"
APIKEY="${LIVE_TENANT_API_KEY}"
...
nhas() {
  grep -q "$1" /tmp/m56.json && fail "$2: response leaked $1 — $(head -c 300 /tmp/m56.json)"
  return 0
}

deploy() { # name codefile
  local body
  body="$(python3 -c 'import json,sys;print(json.dumps({"name":sys.argv[1],"code":open(sys.argv[2]).read()}))' "$1" "$2")"
...
for _ in $(seq 1 30); do
  if [[ "$(as_admin GET "/admin/v1/function-triggers/${TRIGGER_ID}/deliveries")" == "200" ]] &&
    grep -q '"status":"success"' /tmp/m56.json; then
    DELIVERED="yes"
    break
  fi
  sleep 1
