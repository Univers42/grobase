...
docker logs "${RT_ON}" 2>&1 | grep -q "realtime metering ON" ||
  {
    ...
    fail "realtime-server did not announce metering ON (line: RT_ON metering log)"
  }
...
OUT="$(drive_ws "${RT_ON}" "${D1} ${D2} ${D3}")" ||
  {
    red "ws driver output:"
    echo "${OUT}"
    fail "WS driver failed to open/close the connections (line: drive_ws A)"
  }
echo "${OUT}" | grep -q "WS_CONNS_CLOSED" ||
  {
    red "ws driver output:"
    echo "${OUT}"
    fail "WS driver did not confirm all connections closed (line: drive_ws A confirm)"
  }
...
[[ "${METERED}" -ge 3 ]] ||
  {
    red "RT_ON logs:"
    docker logs "${RT_ON}" 2>&1 | tail -20
    fail "expected ≥3 close-path metering records, saw ${METERED} (line: METERED < 3)"
  }
...
OUT_OFF="$(drive_ws "${RT_OFF}" "${D1} ${D2} ${D3}")" ||
  {
    red "ws driver output:"
    echo "${OUT_OFF}"
    fail "(B) WS driver failed (line: drive_ws B)"
  }
