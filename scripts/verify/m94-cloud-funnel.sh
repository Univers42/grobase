...
wait_redis() {
  for i in $(seq 1 60); do
    rcli "$1" PING 2>/dev/null | grep -q PONG && return 0
    sleep 0.5
  done
  return 1
}
...
wait_log "${ORCH}" "metering ingest connected" 80 ||
  {
    red "orch logs:"
    docker logs "${ORCH}" 2>&1 | tail -25
    fail "metering ingest never connected (B1 drain)"
  }
wait_log "${ORCH}" "billing enabled" 60 || {
  docker logs "${ORCH}" 2>&1 | tail -25
  fail "BillingReporter never enabled"
}
wait_log "${ORCH}" "spend caps enabled" 60 || {
  docker logs "${ORCH}" 2>&1 | tail -25
  fail "spend-cap guard never enabled"
}
wait_log "${ORCH}" "quota enforcement enabled" 60 || {
  docker logs "${ORCH}" 2>&1 | tail -25
  fail "QuotaGuard never enabled"
}
...
{ docker logs "${TC}" 2>&1 || true; } | grep -q "abuse guard enabled" ||
  {
    ...
    fail "abuse guard never reported enabled"
  }
...
[[ "${EV}" -ge 1 ]] || {
  red "events:"
  cat "${EVENTS_TMP}"
  fail "B3: stripe-mock received 0 meter events (BillingReporter never reported the funnel window)"
}
...
[[ "$(rcli "${REDIS}" SISMEMBER quota:over "${T_OVERQ}")" == "1" ]] ||
  {
    red "quota:over:"
    rcli "${REDIS}" SMEMBERS quota:over
    fail "R1: QuotaGuard never published ${T_OVERQ} to quota:over"
  }
CODE=
...
[[ "$(rcli "${REDIS}" SISMEMBER spend:over "${T_OVERB}")" == "1" ]] ||
  {
    red "spend:over:"
    rcli "${REDIS}" SMEMBERS spend:over
    fail "R2: spend-cap never published ${T_OVERB} to spend:over (the hard-cap decision)"
  }
...
[[ "$(rcli "${REDIS}" SISMEMBER tenant:suspended "${T_ABUSE}")" == "1" ]] ||
  {
    red "tenant:suspended:"
    rcli "${REDIS}" SMEMBERS tenant:suspended
    fail "R3: ${T_ABUSE} absent from Redis tenant:suspended"
  }
...
{ docker logs "${PTC}" 2>&1 || true; } | grep -q "abuse guard disabled" ||
  {
    docker logs "${PTC}" 2>&1 | tail -20
    fail "[PARITY] tenant-control did not report abuse guard disabled (flag default not OFF?)"
  }
{ docker logs "${PORCH}" 2>&1 || true; } | grep -q "billing disabled" ||
  {
    docker logs "${PORCH}" 2>&1 | tail -20
    fail "[PARITY] orchestrator did not report billing disabled (flag default not OFF?)"
  }
...
[[ "$(rcli "${PREDIS}" EXISTS quota:over)" == "0" ]] ||
  {
    red "quota:over:"
    rcli "${PREDIS}" SMEMBERS quota:over
    fail "[PARITY] quota:over was written with the guard OFF — NOT byte-parity"
  }
...
[[ "${PEV}" == "0" ]] || {
  cat "${EVENTS_TMP}"
  fail "[PARITY] stripe-mock received ${PEV} events with BILLING_ENABLED off — expected 0"
}
[[ "$(rcli "${PREDIS}" EXISTS spend:over)" == "0" ]] ||
  {
    red "spend:over:"
    rcli "${PREDIS}" SMEMBERS spend:over
    fail "[PARITY] spend:over was written with the guard OFF — NOT byte-parity"
  }
