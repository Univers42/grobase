...
# **************************************************************************** #
#                                                                              #
#  m190-prod-overlay-hardening.sh — the production overlay's security values   #
...
#            only, runs relay.ts, same profiles as the runtime (m192 = live)   #
...
start_kong() {
  ...
  docker run -d --name "$1" --network none --memory 1g \
    ...
    --entrypoint sh "${KONG_IMG}" -ec 'sed -e "s|__KONG_PUBLIC_API_KEY__|m190-anon|g" -e "s|__KONG_SERVICE_API_KEY__|m190-service|g" \
      -e "s|__KONG_CORS_ORIGIN_[A-Z_]*__|http://localhost|g" -e "s|__JWT_SECRET__|m190-dummy-jwt-secret-m190-dummy-jwt|g" \
...
static_guards
static_held
step "functions network jail (m192 proves it live)"
