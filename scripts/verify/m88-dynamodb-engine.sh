...
C="$(tx_exec insert '{"id":"txa","v":"A"}')"
[[ "${C}" == "200" ]] || fail "(A) tx execute put A got ${C} — $(head -c 200 "${BODY_TMP}") (line: A tx put A)"
C="$(tx_exec insert '{"id":"txb","v":"B"}')"
[[ "${C}" == "200" ]] || fail "(A) tx execute put B got ${C} — $(head -c 200 "${BODY_TMP}") (line: A tx put B)"
