...
docker ps --format '{{.Names}}' | grep -q mini-baas-kong || {
  echo "  ✗ stack not up (run: make up)"
  exit 1
}
...
bash scripts/provision-contract.sh "${CONTRACT}" || {
  echo "  ✗ provisioner failed"
  exit 1
}
...
if [ -f "${OUT}" ] && grep -q "PUBLIC_API_KEY=mbk_" "${OUT}" && grep -q "PUBLIC_DB_ID=${DB_ID}" "${OUT}" && grep -q "PUBLIC_TENANT_ID=smoke" "${OUT}"; then
  ...
  bad "frontend config missing/unresolved"
  [ -f "${OUT}" ] && cat "${OUT}"
fi
...
[ "${FAIL}" -eq 0 ] && echo "✅ m165 PASS — generic contract provisioning works end-to-end" || {
  echo "✗ m165 FAIL"
  exit 1
}
