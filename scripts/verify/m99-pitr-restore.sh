...
pg_val() {
  local c="$1"
  shift
  docker exec -i "$c" psql -U postgres -d postgres -tAc "$1" 2>/dev/null | tr -d '[:space:]'
}
...
BB_LOG="$(docker run --rm --network "${NET}" -e PGPASSWORD="${PGPW}" \
  ...
  {
    red "pg_basebackup output:"
    printf '%s\n' "${BB_LOG}" | tail -12
    fail "pg_basebackup failed — no physical base for PITR (line: pg_basebackup)"
  }
...
PITR_A_LOG="$(docker run --rm \
  ...
  {
    red "pitr-restore output:"
    printf '%s\n' "${PITR_A_LOG}" | tail -15
    fail "(A) pitr-restore.sh failed to prepare the recovery PGDATA (line: A pitr-restore prep)"
  }
...
docker run --rm -u 0 -v "${REC_A_DATA}:/d:ro" "${PG_IMAGE}" \
  ...
  {
    red "auto.conf PITR lines (want T1='${T1}'):"
    docker run --rm -u 0 -v "${REC_A_DATA}:/d:ro" "${PG_IMAGE}" grep -aE "recovery_target_time|restore_command" /d/postgresql.auto.conf 2>&1 | sed 's/^/    /'
    fail "(A) recovery_target_time=T1 not written to postgresql.auto.conf (line: A target_time written)"
  }
...
wait_pg "${PG_REC_A}" || {
  docker logs "${PG_REC_A}" 2>&1 | tail -30
  fail "(A) recovery postgres never became ready (replay/promote failed) (line: A rec ready)"
}
...
B_READY=0
for i in $(seq 1 80); do
  if docker exec "${PG_REC_B}" pg_isready -h 127.0.0.1 -p 5432 -U postgres -d postgres >/dev/null 2>&1; then
    B_READY=1
    break
  fi
