...
TC_IMG="m131-tc-$$:scratch"
AR_IMG="m131-ar-$$:scratch"
NET="m131net-$$"
PG="m131-pg-$$"
TC="m131-tc-$$"
AR="m131-ar-$$"
PORT_TC="${M131_PORT_TC:-18933}"
PORT_AR="${M131_PORT_AR:-18934}"
PGPW="postgres"
...
MID_A="$(json_str id)"
[[ -n "${MID_A}" ]] || fail "no mount id returned (line: A mount id)"
...
emit_gate_log() { (
  ...
  log_event GATE --gate "m131=PASS" --outcome pass \
    ...
    --ref "scripts/verify/m131-selfserve-mount-create.sh" >/dev/null 2>&1
  exit 0
) || true; }
emit_gate_log
