...
skip() {
  printf '\033[1;33m[M46] SKIP: %s\033[0m\n' "$*"
  exit 0
}
...
reg_mount() {
  ...
  [[ "${code}" == "201" ]] || {
    red "mount ${engine}/${name} register HTTP ${code}: $(cat /tmp/m46-mount.json)"
    exit 1
  }
...
live_tenant_provision "sp-a-${TS}" >/dev/null || {
  red "provision A failed"
  exit 1
}
A_KEY="${LIVE_TENANT_API_KEY}"
A_SLUG="${LIVE_TENANT_SLUG}"
A_KEYID="${LIVE_TENANT_KEY_ID}"
A_PG="${LIVE_TENANT_DB_ID}"
KONG="${LIVE_KONG_URL}"
ANON="${LIVE_ANON_APIKEY}"
SERVICE_KEY="${LIVE_SERVICE_APIKEY}"
live_tenant_provision "sp-b-${TS}" >/dev/null || {
  red "provision B failed"
  exit 1
}
B_KEY="${LIVE_TENANT_API_KEY}"
B_SLUG="${LIVE_TENANT_SLUG}"
B_KEYID="${LIVE_TENANT_KEY_ID}"
B_PG="${LIVE_TENANT_DB_ID}"
...
POOLS_BASE="$(metric 'baas_data_plane_pools_open')"
POOLS_BASE="${POOLS_BASE:-0}"
...
[[ "${dcode}" =~ ^20[01]$ || "${dcode}" == 409 ]] || {
  red "mysql create_table HTTP ${dcode}: $(cat /tmp/m46-ddl.json)"
  exit 1
}
...
ins() { # $1 key $2 db_id $3 id $4 label — informative; step 5 is the real gate
  local r
  r="$(q "$1" "$2" "{\"op\":\"insert\",\"resource\":\"${TABLE}\",\"data\":{\"id\":\"$3\",\"label\":\"$4\"}}")"
...
iso() { # $1 desc $2 key $3 db_id $4 own $5 other
  local r
  r="$(q "$2" "$3" "{\"op\":\"list\",\"resource\":\"${TABLE}\",\"limit\":50}")"
  if ! grep -q "$4" <<<"${r}"; then
    red "$1: missing OWN row '$4' — resp: ${r}"
    fail=1
    return
  fi
  if grep -q "$5" <<<"${r}"; then
    red "$1: CROSS-TENANT LEAK — saw '$5' — resp: ${r}"
    fail=1
    return
  fi
...
POOLS_OPEN="$(metric 'baas_data_plane_pools_open')"
POOLS_OPEN="${POOLS_OPEN:-0}"
...
if [[ "${DELTA}" != "${WANT}" ]]; then
  red "collapse mismatch: ${DELTA} pools held, expected ${WANT}"
  fail=1
fi
...
[[ "${fail}" == 0 ]] || {
  red "cross-engine SHARE_POOLS probe FAILED (EXPECT=${EXPECT})"
  exit 1
}
