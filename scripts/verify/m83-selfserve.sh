...
step "4e2/8 (A · CONTAINMENT, H-13) a write-only key cannot revoke an admin key; an admin key can"
C="$(me_req POST "${PORT_ON}" /v1/tenants/me/keys "${KEY_A}" "{\"name\":\"m83-write-only-$$\",\"scopes\":[\"read\",\"write\"]}")"
[[ "${C}" == "200" || "${C}" == "201" ]] || fail "(A) mint write-only key expected 200/201, got ${C} — $(head -c 300 "${BODY_TMP}") (line: H-13 mint write)"
WRITE_KEY="$(json_str key)"
C="$(me_req POST "${PORT_ON}" /v1/tenants/me/keys "${KEY_A}" "{\"name\":\"m83-admin-2-$$\",\"scopes\":[\"admin\"]}")"
[[ "${C}" == "200" || "${C}" == "201" ]] || fail "(A) mint second admin key expected 200/201, got ${C} — $(head -c 300 "${BODY_TMP}") (line: H-13 mint admin)"
ADMIN2_ID="$(json_str id)"
C="$(me_req DELETE "${PORT_ON}" "/v1/tenants/me/keys/${ADMIN2_ID}" "${WRITE_KEY}")"
[[ "${C}" == "403" ]] || fail "(A) a read+write key revoking an ADMIN key got ${C}, want 403 — a lesser key can lock the tenant's admins out (line: H-13 revoke containment)"
[[ "$(psql_val "SELECT revoked_at IS NULL FROM public.tenant_api_keys WHERE id='${ADMIN2_ID}'")" == "t" ]] ||
  fail "(A) the admin key was revoked despite the refusal (line: H-13 DB)"
C="$(me_req DELETE "${PORT_ON}" "/v1/tenants/me/keys/${ADMIN2_ID}" "${KEY_A}")"
