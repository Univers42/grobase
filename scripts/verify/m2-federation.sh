...
if [[ ${LIVE} -eq 1 ]]; then
  ...
  # shellcheck disable=SC2043  # intentional single-svc loop (only query-router runs the signed blocks)
...
SH
  ...
  mysql_db_id=$(
    docker compose -f "${COMPOSE_FILE}" exec -T -e M2_USER_ID="${M2_USER_ID}" -e M2_MYSQL_DSN="${mysql_dsn}" query-router node --input-type=module - <<'NODE'
...
NODE
  ...
  redis_db_id=$(
    docker compose -f "${COMPOSE_FILE}" exec -T -e M2_USER_ID="${M2_USER_ID}" query-router node --input-type=module - <<'NODE'
const { signedFetch } = await import('file:///tmp/_signed-fetch.mjs');
const userId = process.env.M2_USER_ID;
const body = {
  engine: 'redis',
