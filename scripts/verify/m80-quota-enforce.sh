...
PUBLISHED=
for i in $(seq 1 60); do
  if [[ "$(redis_cli SISMEMBER quota:over "${TENANT_OVER}" 2>/dev/null)" == "1" ]]; then
    PUBLISHED=1
    break
  fi
  sleep 0.5
...
[[ -n "${PUBLISHED}" ]] || {
  ...
  fail "OVER tenant never appeared in quota:over (the guard's decision) (line: wait quota:over)"
}
