...
source "${SCRIPT_DIR}/../lib/lib-bench.sh"
...
git clone --depth 1 --branch "${SUPABASE_REF}" https://github.com/supabase/supabase.git "${SB_DIR}" >/dev/null 2>&1 ||
  {
    red "clone failed — check the pinned ref ${SUPABASE_REF}, or run on-demand with network access"
    exit 1
  }
cd "${SB_DIR}/docker"
cp .env.example .env
...
docker compose up -d 2>&1 | tail -3 || {
  red "supabase compose up failed"
  exit 1
}
...
sb=()
for _ in $(seq 1 "${N}"); do sb+=("$(awk -v t="$(probe "${SB_KONG}" "${SB_ANON}")" 'BEGIN{printf "%.2f",t*1000}')"); done
...
if [[ -n "${OUR_ANON}" ]] && docker ps --format '{{.Names}}' | grep -q '^mini-baas-postgres$'; then
  PGU="$(grep -E '^POSTGRES_USER=' "${OUR_INFRA}/.env" | cut -d= -f2-)"
  PGU="${PGU:-postgres}"
  PGD="$(grep -E '^POSTGRES_DB=' "${OUR_INFRA}/.env" | cut -d= -f2-)"
  PGD="${PGD:-postgres}"
...
SQL
  ...
  our=()
  for _ in $(seq 1 "${N}"); do our+=("$(awk -v t="$(probe "${OUR_KONG}" "${OUR_ANON}")" 'BEGIN{printf "%.2f",t*1000}')"); done
