...
[[ -f "${JSONL}" ]] || {
  red "no tenant file ${JSONL} (run make scale-seed SCALE=${SCALE})"
  exit 1
}
...
N="$(jq 'length' "${TLIST}")"
((N > 0)) || {
  red "no usable tenants in ${JSONL}"
  exit 1
}
...
echo "${OUT}" | grep -qE 'HTTP20[01]|HTTP409' || {
  red "shared table create failed: ${OUT}"
  exit 1
}
