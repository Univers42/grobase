...
live_tenant_provision() { # $1 slug (must match ^[a-z0-9][a-z0-9_-]{1,62}$)
  local slug="$1"
  [[ -n "${slug}" ]] || {
    echo "live_tenant_provision: slug required" >&2
    return 1
  }
  ...
  [[ -n "${kong_port}" ]] || {
    echo "kong host port not found (is the stack up?)" >&2
    return 1
  }
  [[ -n "${tc_port}" ]] || {
    echo "tenant-control host port not found" >&2
    return 1
  }
  ...
  [[ -n "${LIVE_SERVICE_TOKEN}" ]] || {
    echo "INTERNAL_SERVICE_TOKEN not found on tenant-control" >&2
    return 1
  }
  [[ -n "${LIVE_ANON_APIKEY}" && -n "${LIVE_SERVICE_APIKEY}" ]] ||
    {
      echo "Kong consumer keys not found on mini-baas-kong" >&2
      return 1
    }
  ...
  [[ "${code}" == "201" || "${code}" == "409" ]] ||
    {
      echo "tenant create failed (${code}): $(cat /tmp/lt-tenant.json)" >&2
      return 1
    }
  ...
  code=$(curl -s -o /tmp/lt-key.json -w '%{http_code}' -X POST \
    ...
    -d "${kbody}")
  [[ "${code}" == "201" ]] || {
    echo "key mint failed (${code}): $(cat /tmp/lt-key.json)" >&2
    return 1
  }
  LIVE_TENANT_API_KEY="$(_lt_json_field key </tmp/lt-key.json)"
  LIVE_TENANT_KEY_ID="$(_lt_json_field id </tmp/lt-key.json)"
  [[ "${LIVE_TENANT_API_KEY}" == mbk_* ]] || {
    echo "minted key has unexpected shape" >&2
    return 1
  }
  ...
  [[ "${code}" == "201" ]] || {
    echo "mount register failed (${code}): $(cat /tmp/lt-mount.json)" >&2
    return 1
  }
  LIVE_TENANT_DB_ID="$(_lt_json_field id </tmp/lt-mount.json)"
  [[ -n "${LIVE_TENANT_DB_ID}" ]] || {
    echo "mount register returned no id" >&2
    return 1
  }
