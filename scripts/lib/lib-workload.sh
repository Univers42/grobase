...
# Requires lib-bench.sh + scripts/lib/lib-live-tenant.sh sourced first and
...
bw_setup_table() {
  ...
  echo "${out}" | grep -q 'HTTP20[01]' || {
    echo "create_table failed: ${out}" >&2
    return 1
  }
  ...
  for chunk in $(seq 0 49); do
    ...
    echo "${out}" | grep -q 'HTTP20[01]' || {
      echo "seed batch ${chunk} failed: ${out}" >&2
      return 1
    }
  done
  return 0
