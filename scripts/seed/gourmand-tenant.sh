...
fail() {
  printf '\033[0;31m[gourmand-tenant] FAIL: %s\033[0m\n' "$*" >&2
  exit 1
}
...
if [[ -z "${DSN}" ]]; then
  DSN="$(env_dsn DIRECT_URL)"
  DSN_SOURCE="Back/.env DIRECT_URL"
fi
if [[ -z "${DSN}" ]]; then
  DSN="$(env_dsn DATABASE_URL)"
  DSN_SOURCE="Back/.env DATABASE_URL"
  if [[ "${DSN}" == *":6543/"* ]]; then
