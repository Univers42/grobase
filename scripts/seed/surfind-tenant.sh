#!/usr/bin/env bash
# **************************************************************************** #
#  surfind-tenant.sh — add a MongoDB plane to the Surfind Spain app on Grobase #
#                                                                             #
#  The surf directory's core (beaches/articles/surf_reports/ratings) runs on  #
#  PostgREST+Postgres. This script bolts on the DOCUMENT engine to showcase   #
#  Grobase's multi-engine reach from one frontend: every surfer keeps a       #
#  private "Bitácora" of surf sessions stored as MongoDB documents,           #
#  owner-scoped per GoTrue user through the query-router                       #
#  (POST /query/v1/{dbId}/tables/surf_sessions).                               #
#                                                                             #
#  It provisions a `surfind` tenant, an enterprise entitlement spanning both  #
#  engines, an mbk_ API key, and a `surfind-mongo` mount → the surfind Mongo  #
#  database, then emits the mongo dbId + app key into web/.env for the SPA.    #
#                                                                             #
#  Idempotent: re-runs reuse the key + mount (probed live) and re-seed the    #
#  demo visitor's bitácora. State → .surfind-tenant.env (gitignored).         #
# **************************************************************************** #
set -euo pipefail
...
STATE_ENV="${REPO_ROOT}/.surfind-tenant.env"
WEB_ENV="${REPO_ROOT}/vendor/surfind-spain/web/.env"
MONGO_CTN="mini-baas-mongo"
SURF_DB="surfind"
MONGO_MOUNT_NAME="surfind-mongo"
TENANT_SLUG="surfind"
DEMO_EMAIL="visitor@surfind.es"
DEMO_PASSWORD="${SURFIND_PASSWORD:-surf-1234}"

cyan() { printf '\033[0;36m[surfind-tenant] %s\033[0m\n' "$*"; }
fail() {
  printf '\033[0;31m[surfind-tenant] FAIL: %s\033[0m\n' "$*" >&2
  exit 1
}
urlenc() { python3 -c "import sys, urllib.parse; print(urllib.parse.quote(sys.argv[1], safe=''))" "$1"; }

# gw_q DBID TABLE BODY [BEARER] — query-router CRUD; body→/tmp/surfind-q.json, echoes status.
gw_q() {
  local extra=()
  [[ -n "${4:-}" ]] && extra=(-H "Authorization: Bearer $4")
  curl -s -o /tmp/surfind-q.json -w '%{http_code}' -X POST "${KONG_URL}/query/v1/$1/tables/$2" \
...
tbody="{\"id\":\"${TENANT_SLUG}\",\"name\":\"Surfind Spain\"}"
svc_auth POST /v1/tenants "${tbody}"
code=$(curl -s -o /tmp/surfind-tenant.json -w '%{http_code}' -X POST "${TC_URL}/v1/tenants" \
  "${SVC_AUTH[@]}" -H 'Content-Type: application/json' -d "${tbody}")
[[ "${code}" == "201" || "${code}" == "409" ]] || fail "tenant create (${code}): $(cat /tmp/surfind-tenant.json)"
...
ebody='{"ceiling_plan":"enterprise","status":"active","entitlement":{"label":"surfind","engines":["postgresql","mongodb"],"capabilities":{"read":true,"write":true,"insert":true,"update":true,"delete":true,"upsert":true,"batch":true,"aggregate":true},"limits":{"rps":250,"burst":500,"max_rows":1000,"quota.query.count":5000000},"max_mounts":2}}'
svc_auth PUT "/v1/tenants/${TENANT_SLUG}/entitlement" "${ebody}"
curl -s -o /tmp/surfind-ent.json -w '%{http_code}' -X PUT \
...
register_mount() {
  ...
  if [[ "${code}" == "201" ]]; then
    ...
    fail "mount $1 register failed (${code}): $(cat /tmp/surfind-mount.json)"
  fi
}
...
if [[ "${key_ok}" == "1" ]]; then
  ...
  kbody='{"name":"surfind-app","scopes":["read","write"]}'
  svc_auth POST "/v1/tenants/${TENANT_SLUG}/keys" "${kbody}"
  code=$(curl -s -o /tmp/surfind-key.json -w '%{http_code}' -X POST \
    "${TC_URL}/v1/tenants/${TENANT_SLUG}/keys" \
    "${SVC_AUTH[@]}" -H 'Content-Type: application/json' -d "${kbody}")
  [[ "${code}" == "201" ]] || fail "key mint (${code}): $(cat /tmp/surfind-key.json)"
  API_KEY="$(_lt_json_field key </tmp/surfind-key.json)"
  KEY_ID="$(_lt_json_field id </tmp/surfind-key.json)"
  [[ "${API_KEY}" == mbk_* ]] || fail "minted key has unexpected shape"

  cyan "registering MongoDB mount '${MONGO_MOUNT_NAME}' → ${SURF_DB}"
  MONGO_DB_ID="$(register_mount "${MONGO_MOUNT_NAME}" mongodb \
    "mongodb://$(urlenc "${MONGO_USER}"):$(urlenc "${MONGO_PASS}")@mongo:27017/${SURF_DB}?authSource=admin")"
  [[ -n "${MONGO_DB_ID}" ]] || fail "mount registration returned empty id"
fi
cyan "mount: mongo=${MONGO_DB_ID}"

# ── 3) seed the demo visitor's bitácora (owner-scoped via their JWT) ─────────
...
if [[ -n "${DEMO_TOK}" ]]; then
  cyan "clearing + seeding the demo visitor's surf_sessions (owner-scoped)"
  gw_q "${MONGO_DB_ID}" surf_sessions '{"op":"delete","filter":{"seed":{"$eq":true}}}' "${DEMO_TOK}" >/dev/null 2>&1 || true
  seed_session() {
    gw_q "${MONGO_DB_ID}" surf_sessions \
...
# web/.env already carries VITE_BAAS_ENDPOINT (same-origin proxy) +
# VITE_BAAS_API_KEY (the Kong anon key for PostgREST/GoTrue). Append the mongo
# coordinates without clobbering those.
cyan "writing mongo coordinates into web/.env"
touch "${WEB_ENV}"
grep -v -E '^VITE_BAAS_(MONGO_DBID|APP_KEY)=' "${WEB_ENV}" >"${WEB_ENV}.tmp" || true
...
cat >"${STATE_ENV}" <<EOF
# generated by scripts/seed/surfind-tenant.sh — DO NOT COMMIT
SURFIND_TENANT_SLUG=${TENANT_SLUG}
SURFIND_API_KEY=${API_KEY}
SURFIND_KEY_ID=${KEY_ID}
SURFIND_MONGO_DB_ID=${MONGO_DB_ID}
SURFIND_DB_NAME=${SURF_DB}
SURFIND_KONG_URL=${KONG_URL}
SURFIND_ANON_APIKEY=${ANON_KEY}
SURFIND_SERVICE_APIKEY=${SERVICE_KEY}
EOF
