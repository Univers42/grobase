#!/bin/sh
# hypertube-users.sh — seed ~8 real demo users + public profiles + comments.
# Loads the gitignored .hypertube-baas.env state, then runs the zero-dep Node
# seeder inside a node:20-alpine container on the mini-baas docker network (so
# it reaches Kong at mini-baas-kong:8000, not the host loopback). Idempotent.
set -eu
...
# run_seeder invokes the users seeder in a node:20-alpine container.
run_seeder() {
  docker run --rm --network "$NET" \
    -e HT_KONG_URL="$KONG_IN_NET" \
    -e HT_ANON_APIKEY="$HT_ANON_APIKEY" \
    -e HT_SERVICE_APIKEY="$HT_SERVICE_APIKEY" \
    -e HT_API_KEY="$HT_API_KEY" \
    -e HT_MONGO_DB_ID="$HT_MONGO_DB_ID" \
    -v "${REPO_DIR}:/r" -w /r \
    node:20-alpine node scripts/seed/hypertube-users.mjs
}
