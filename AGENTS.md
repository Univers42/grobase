...
The `claude-deal-with-the-devil/` directory (a git submodule) is the upstream of the `.claude/` config. Its tools, hooks, extra agents (`builder`, `forger`, `innovator`), skills and rules are **merged into `.claude/`** (grobase's own versions of the files both trees share were kept). Run `bash .claude/tools/selfcheck.sh --summary` after editing `.claude/`; hooks live in `.claude/settings.json`, MCP servers in the root `.mcp.json`.
...
- **`claude-deal-with-the-devil/`** is a git submodule (the upstream `.claude/` config); **not** an app migration target.
