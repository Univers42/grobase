...
restore_postgres() {
  ...
  have_container "$c" || {
    warn "postgres not running — skipped"
    return
  }
  hdr "PostgreSQL"
...
restore_mysql() {
  ...
  have_container "$c" || {
    warn "mysql not running — skipped"
    return
  }
  hdr "MySQL"
...
restore_mongo() {
  ...
  have_container "$c" || {
    warn "mongo not running — skipped"
    return
  }
  hdr "MongoDB"
...
restore_mssql() {
  ...
  have_container "$c" || {
    warn "mssql not running — skipped"
    return
  }
  hdr "MSSQL"
...
restore_dynamodb() {
  ...
  have_container "$c" || {
    warn "dynamodb not running — skipped"
    return
  }
  hdr "DynamoDB"
...
restore_minio() {
  ...
  have_container "$c" || {
    warn "minio not running — skipped"
    return
  }
  hdr "MinIO"
  [ -f "$IN/minio/objects.tar.gz" ] || {
    warn "no minio archive"
    return
  }
