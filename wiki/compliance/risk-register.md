...
| R-O5 | Stored secrets / credential leak / Vault not *enforced* — plaintext DSNs possible outside `max` | Med × High | Mitigate — done in code: under `max` inline DSNs are refused (403) and `credential_ref{provider:vault}` resolves per request (m121, nightly); other tiers encrypt inline DSNs at rest | gate `m121`, [`../security-audit.md`](../security/security-audit.md) (audit O5) | Control-plane | Med | 2026-Q4 |
...
| R-G-Net | Network / lateral movement / flat bridge — engines and vault shared the app bridge with the edge | Med × Med | Mitigate — done for compose: `docker-compose.netseg.yml` is the default in `make prod-up` (engines on `net-data`, vault on `net-vault`; `PROD_NETSEG=0` opts out); Helm NetworkPolicies off until enabled on a cluster | gate `m66`, gate `m106`, [`../security-audit-asvs.md`](../security/security-audit-asvs.md) (G-Net) | Platform | Med | 2026-Q4 |
...
| R-G-Rotate | Key management / stale credentials / no atomic (no-restart) key-rotation primitive for `JWT_SECRET`/service-token | Low × Med | Mitigate — service-token half done (`rotate-service-token.sh` begin/swap/finish, both tokens accepted in the window, gates m205 + m68); `JWT_SECRET` half needs gotrue + PostgREST dual-key support, staging first | [`../security-residuals-runbook.md`](../security/security-residuals-runbook.md) (G-Rotate) | Platform | Low | 2027-Q1 |
