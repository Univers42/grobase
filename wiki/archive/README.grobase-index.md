...
| # | Doc | What it answers |
|---|---|---|
| 00 | [Overview](../architecture/00-overview.md) | What the BaaS is, the 3-language plane pattern, how a request flows, the layer model today |
| 01 | [Gap analysis](../architecture/01-gap-analysis.md) | The 11 concrete gaps (G1–G11) between "works" and "swappable-layer product", prioritised with evidence |
| 02 | [Layer & edition model](../architecture/02-layer-edition-model.md) | The manifest: planes → profiles, editions → planes, engine + isolation + product-mode axes |
| 03 | [Control plane (Go)](../architecture/03-control-plane.md) | Provisioning brain, gateway/admin API, shadow→enabled cutover, credential providers |
| 04 | [Data plane (Rust)](../architecture/04-data-plane.md) | Capability-aware planner, pluggable isolation strategies, credential seam, transactions |
| 05 | [Orchestration, observability & roadmap](../architecture/05-orchestration-observability-roadmap.md) | The Makefile orchestrator, cross-tier o11y, parity gate, SDK, packaging, P0–P7 roadmap |
| 06 | [Product assessment](../architecture/06-product-assessment.md) | Honest evaluation: is it a *good product* yet? (verdict: strong chassis, engine half-built) |
...
| Doc | What it answers |
|---|---|
| [Competitive matrix](../competitive/competitive-matrix.md) | Feature-parity vs **Supabase + Firebase** (~91 rows) + the differentiators neither has |
| [Grobase vs Supabase — offer](../competitive/grobase-vs-supabase-offer.md) | Service-for-service map with **measured** footprint/latency (lighter + faster, like-for-like) |
| [Marketability readiness](../go-to-market/marketability-readiness.md) | The four launch bars (parity / scale-SLO / security / live-signup) as checkable gates |
| [Roadmap to market](../go-to-market/roadmap-to-market.md) | OSS-first phased plan (Track A → OSS launch; Tracks B+C → managed cloud) |
| [Security audit (ASVS)](../security/security-audit-asvs.md) | OWASP ASVS L1/L2 + SOC2-lite control map + open residuals |
| [Migrate from Supabase](../guides/migrate-from-supabase.md) | Dependency-swap guide — the SDK is Supabase-shaped |
| [Migrate from Firebase](../guides/migrate-from-firebase.md) | Cross-paradigm translation (Firestore/RTDB/Rules → engines + RLS) |
