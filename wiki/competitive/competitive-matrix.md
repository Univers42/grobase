...
- [marketability-readiness.md](../go-to-market/marketability-readiness.md) — the four marketability bars and where we stand.
- [roadmap-to-market.md](../go-to-market/roadmap-to-market.md) — Phase 0 + Track A (OSS) + Track B (cloud) + Track C (scale/HA).

Related: [grobase-master-plan.md](../go-to-market/grobase-master-plan.md), [product-plan/06-saas-multitenancy-quotas-billing.md](../product-plan/06-saas-multitenancy-quotas-billing.md), [product-plan/09-100k-tenant-path.md](../product-plan/09-100k-tenant-path.md), [security-audit.md](../security/security-audit.md), [cost-analysis.md](../cost-and-tiers/cost-analysis.md), [offer-sheet-v2.md](../go-to-market/offer-sheet-v2.md), [nano-vs-pocketbase.md](./nano-vs-pocketbase.md).
...
| 47 | Flutter / Dart | v | v | **[~]** | — **GENERATED / experimental, not a first-class hand-written SDK** (no convenience facade). **LIVE (gate m58)**: `sdk-dart` `dart pub get` OK + `dart analyze --fatal-infos` clean; exposes all 5 Api surfaces and **32 distinct operations == the spec (32) == python (32)**. Residual: not yet hand-polished/pub-published | M | P1 | `sdk-dart/`, `sdk/scripts/codegen-polyglot.sh`; `scripts/verify/m58-sdks-compile.sh` · gate m58 |
| 48 | Swift / iOS | v | v | **[x]** | No Swift SDK | L | P2 | — |
| 49 | Kotlin / Android | v | v | **[x]** | No Kotlin SDK | L | P2 | — |
| 50 | Python | v | v | **[~]** | — **GENERATED / experimental, not a first-class hand-written SDK** (ships a small hand-written `grobase.client.Client` facade as the reference pattern, but not feature-complete). **LIVE (gate m58)**: `sdk-python` `pip install` + import of all 5 Api surfaces in python:3.12; exposes **32 operations == the spec (32) == dart (32)**. Residual: not yet pip-published | M | P1 | `sdk-python/`, `sdk/scripts/codegen-polyglot.sh`; `scripts/verify/m58-sdks-compile.sh` · gate m58 |
...
| # | Capability | Supabase | Firebase | Grobase | Gap | Effort | Pri | Notes / anchor |
|---|-----------|:--------:|:--------:|:-------:|-----|:------:|:---:|----------------|
| 77 | OSS / self-hostable (prod) | v | x | **[+]** | — OSS, Docker-Compose-first, multi-arch on Docker Hub; **tiny footprint** (nano 5.16MB, essential ~660 MiB / 13 services[†]) beats both. **Packaging live-proven (gate m61)**: substantive Supabase/Firebase migration guides + one-command Makefile bring-up | — | P0 | [offer-sheet-v2.md](../go-to-market/offer-sheet-v2.md); `scripts/verify/m61-packaging.sh` · gate m61 |
...
| # | Capability | Supabase | Firebase | Grobase | Gap | Effort | Pri | Notes / anchor |
|---|-----------|:--------:|:--------:|:-------:|-----|:------:|:---:|----------------|
| 82 | Free tier | v | v | **[~]** | Self-host is free; no managed free tier yet (no billing system) | L | P1 | [offer-sheet-v2.md](../go-to-market/offer-sheet-v2.md) |
...
| 86 | Dedicated compute add-on | v | x | **[~]** | Tiers basic→max exist as shapes; no self-serve dedicated-compute purchase | L | P2 | [service-tiers.md](../cost-and-tiers/service-tiers.md) |
...
| D4 | **Per-tenant cost efficiency at idle** | per-project min cost | per-project | Marginal ~$0.40–1.00/tenant on a shared pro host; data path 3.3 MiB Rust vs 127 MiB Node (~38×) — [cost-analysis.md](../cost-and-tiers/cost-analysis.md) |
...
These are the [v]/[~] table-stakes that block a credible OSS self-host launch (full detail in [roadmap-to-market.md](../go-to-market/roadmap-to-market.md), Track A):
...
| 4 | 47, 50 | ✅ **LIVE (A4, gate m58 PASS)** — OpenAPI spec + **generated/experimental Python & Dart clients install/import clean, 32 ops == spec** (rows 47, 50 → `[~]` — generated, not first-class hand-written); Swift/Kotlin next | (was the single blocker for all multi-language SDKs) | M |
...
| 8 | — | ✅ **posture LIVE-proven (A6, gate m60 PASS)** — SHIPPED controls hold live (anon→internal 401/42501 · forged JWT 401 · cross-tenant mount 404) + CI security gates wired (gitleaks·cargo-audit·govulncheck·trivy·semgrep·trufflehog·zap) + ASVS/SOC2-lite map; **residual (7 KNOWN-OPEN):** RS256 issuer + per-deployment keys, header-trust/network, no certs | Launch gate; residuals are launch-blockers ([security-audit.md](../security/security-audit.md)) | M |

> Not P0 but high-leverage for the *cloud* launch (Track B): metering (84), billing/Stripe (83), per-tenant observability (57, 64, 65), tenant self-service dashboard (56, 57). These are the largest net-new build — see [marketability-readiness.md](../go-to-market/marketability-readiness.md) Bar 4.

### Canonical gate numbers for the work this matrix scopes

New gates are numbered **above the shipped m1–m53 range** (never reuse a shipped number). The closing work in [roadmap-to-market.md](../go-to-market/roadmap-to-market.md) maps as:
...
| **m58** ✅ PASS | A4 — multi-language SDKs (rows 47, 50→`[~]`, generated/experimental): python install/import + dart clean analyze + 32 ops == spec == python == dart |
...
**Wiki docs (relative):**
- [marketability-readiness.md](../go-to-market/marketability-readiness.md) · [roadmap-to-market.md](../go-to-market/roadmap-to-market.md) · [grobase-master-plan.md](../go-to-market/grobase-master-plan.md)
...
- [security-audit.md](../security/security-audit.md) · [cost-analysis.md](../cost-and-tiers/cost-analysis.md) · [offer-sheet-v2.md](../go-to-market/offer-sheet-v2.md) · [nano-vs-pocketbase.md](./nano-vs-pocketbase.md) · [service-tiers.md](../cost-and-tiers/service-tiers.md)
