...
> The roadmap that turns the platform from "a beautifully engineered chassis with the engine half-built" (see [../06-product-assessment.md](../architecture/06-product-assessment.md)) into the product in the vision: an **engine-agnostic** BaaS that does **all operations, not just CRUD**, and can run as an **OLAP** *or* **OLTP** model — switchable as a runtime context, not just a deploy shape.
