...
- **[competitive-matrix.md](../competitive/competitive-matrix.md)** — the 91-row capability matrix vs Supabase/Firebase.
  ...
  **big data disk** (`BENCH_WORK_BASE=/mnt/storage/bench`) so Supabase's
  ...
  - **Footprint (like-for-like):** Supabase **2884 MiB / 13 containers** vs the
    **Grobase parity shape ~448 MiB** (Postgres+auth+REST+realtime+storage+functions+gateway;
    ...
    [grobase-vs-supabase-offer.md](../competitive/grobase-vs-supabase-offer.md).
...
- **A2 Functions DX + A5 GraphQL/realtime — LIVE (gates m56 + m59 PASS, v1.1.0 wave).**
  ...
  - **Adversarial review (14-agent workflow, find→verify) — 8 real findings, 0 dismissed,
    ...
    *authorization*, so a token scoped to `tenant_a` could broadcast / publish / inject
    ...
    itself the audit-ready posture in action (see [security-audit.md](../security/security-audit.md)).
...
[cost-analysis.md](../cost-and-tiers/cost-analysis.md) ·
[offer-sheet-v2.md](./offer-sheet-v2.md) ·
[nano-vs-pocketbase.md](../competitive/nano-vs-pocketbase.md)
...
- **★ CLOUD LAUNCH + GTM** — gated by **B (full)** *and* **C (full)**. A stranger signs up → gets a
  ...
  [cost-analysis.md](../cost-and-tiers/cost-analysis.md)), signup funnel, legal (ToS/privacy/DPA), SLA, status page.
...
- **Key files / anchors:** [security-audit.md](../security/security-audit.md), `go control-plane
...
- **Exit gate:** **m60** audit-ready — all listed residuals closed and re-verified; controls mapped to
  an ASVS/SOC2-lite checklist in [security-audit.md](../security/security-audit.md); `cargo-audit` + `govulncheck`
...
- **Why:** Closes the billing gap (no Stripe today) and the **revenue half of BAR 4** ("be billed").
  ...
  economics from [cost-analysis.md](../cost-and-tiers/cost-analysis.md) (marginal ~$0.40–1.00/tenant on a shared pro host).
...
- **Exit gate:** **m71** 100K-SLO — 100K tenants seeded and load-tested on the horizontal topology;
  ...
  ([offer-sheet-v2.md](./offer-sheet-v2.md) + [cost-analysis.md](../cost-and-tiers/cost-analysis.md)), signup funnel
...
[competitive-matrix.md](../competitive/competitive-matrix.md) is left unowned.
...
- Gap numbers (#1..#91) and status glyphs → **[competitive-matrix.md](../competitive/competitive-matrix.md)**
...
- Security baseline + residuals → [security-audit.md](../security/security-audit.md)
- Pricing + unit economics → [offer-sheet-v2.md](./offer-sheet-v2.md), [cost-analysis.md](../cost-and-tiers/cost-analysis.md)
- nano/PocketBase wedge honesty → [nano-vs-pocketbase.md](../competitive/nano-vs-pocketbase.md)
