...
bash claude-deal-with-the-devil/tools/digest.sh
...
### Available MCP servers (in `.mcp.json` of `claude-deal-with-the-devil/`)
...
**Wire them:** copy `claude-deal-with-the-devil/.mcp.json` to `.claude/.mcp.json` if
it isn't already there, so these servers load automatically.
...
### Available tools from `claude-deal-with-the-devil/tools/`

These are bash scripts that replace manual exploration. Run them, don't re-derive
what they already answer:

```bash
bash claude-deal-with-the-devil/tools/digest.sh         # full repo brief (cached)
bash claude-deal-with-the-devil/tools/facts.sh          # toolchain + test commands
bash claude-deal-with-the-devil/tools/preflight.sh      # env/secrets/toolchain check
bash claude-deal-with-the-devil/tools/codemap.sh        # where things live, what's heavy
bash claude-deal-with-the-devil/tools/untested.sh       # files with no test coverage
bash claude-deal-with-the-devil/tools/dupes.sh          # duplicate code blocks
bash claude-deal-with-the-devil/tools/quality.sh --with-tests   # full quality gate
bash claude-deal-with-the-devil/tools/watch.sh --idle 60 -- make build  # safe build
bash claude-deal-with-the-devil/tools/selfcheck.sh      # verify .claude/ config integrity
```
...
```bash
# Check if .claude/.mcp.json exists with the MCP servers
ls .claude/.mcp.json 2>/dev/null || cp claude-deal-with-the-devil/.mcp.json .claude/.mcp.json

# Run selfcheck on the .claude/ config
bash claude-deal-with-the-devil/tools/selfcheck.sh
...
Standard: follows `claude-deal-with-the-devil/tools/lib/common.sh` pattern.
...
| Tool index | `claude-deal-with-the-devil/tools/README.md` |
| Hooks (enforcement layer) | `claude-deal-with-the-devil/hooks/HOOKS-README.md` |
