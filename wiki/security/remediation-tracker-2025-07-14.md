...
| **H-19** | `IDENTITY_HEADER_MODE=compat` everywhere but fly. | The signer half is now built and proven (see H-19a below), so strict is achievable; the prod overlay still does not set it (m195 asserts that) because the flip changes what production accepts. Checklist in `config.env`: every caller must arrive with a Bearer JWT or an api key rather than only Kong's raw `X-User-*`. The other two items on that checklist are now closed — `GOTRUE_JWT_ISSUER` is passed to all 14 services that verify a user token (`app-services.yml`, `auth-api.yml`), and `outbox-relay`'s missing `JWT_SECRET` is a non-issue because its only route is an unguarded `@Controller('health')`. | Flip `IDENTITY_HEADER_MODE=strict` in `docker-compose.prod.yml` and invert `m195-prod-overlay-hardening.sh:241-242`. Gated on `m202-strict-identity-serviceable.sh` going green: recreate storage-router with `IDENTITY_HEADER_MODE=strict` and run it. The gate fails loudly in compat, so it cannot pass vacuously. |
...
| **H-19b** | Nothing proved that strict mode works for real traffic — the unit suite exercises the library, not a service behind Kong with a live GoTrue token. | `m202-strict-identity-serviceable.sh`: against a storage-router recreated in strict mode (the hardest case — it has no signer of its own), a real GoTrue session is accepted while a raw `X-User-Id`, a cross-app realtime token whose `sub` is a tenant slug, and a token signed with another secret are each refused; statically, every user-token verifier must agree on one `GOTRUE_JWT_ISSUER`. | The gate refuses to run in compat (`FAIL — runs IDENTITY_HEADER_MODE='compat'`), so it cannot pass vacuously; verified red on the current compat stack. NOT yet green: recreating a service in strict needs an assembled `.env`, which this machine does not currently have. |
