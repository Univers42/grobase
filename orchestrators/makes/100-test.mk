...
# ── Linters: one per language/format. test-lint runs them ALL and shows each. ──
# Errors fail; advisory warnings (clippy lints, checkmake notes) are shown, not fatal.
LINT_KINDS := test-lint-shell test-lint-rust test-lint-go test-lint-ts test-lint-yaml test-lint-docker test-lint-make

test-lint-shell: ## Lint shell — bash -n (all) + shellcheck -S error
	@rc=0; for f in $$(git ls-files '*.sh' 2>/dev/null); do bash -n "$$f" || { echo -e "$(_R)  parse: $$f$(_0)"; rc=1; }; done; \
	if command -v shellcheck >/dev/null 2>&1; then \
		for f in $$(git ls-files '*.sh' 2>/dev/null); do shellcheck -S error -e SC1128 "$$f" || rc=1; done; \
	else echo -e "$(_D)  (host shellcheck absent — bash -n only)$(_0)"; fi; \
	[ $$rc -eq 0 ] && echo -e "$(_G)✓ shell$(_0)" || exit 1

test-lint-rust: _rust-toolchain ## Lint Rust — cargo clippy (data-plane workspace, in Docker)
	@$(CARGO_DPR) sh -c 'rustup component add clippy >/dev/null 2>&1 || true; cargo clippy --workspace --all-targets 2>&1' \
		&& echo -e "$(_G)✓ rust clippy$(_0)"
...
test-lint-yaml: ## Lint YAML — yamllint (relaxed; errors fail, in Docker)
	@docker run --rm -v "$(CURDIR)":/d -w /d cytopia/yamllint:latest \
		-d "{extends: relaxed, rules: {line-length: disable, comments: disable, document-start: disable, truthy: disable, empty-lines: disable, indentation: disable, brackets: disable, commas: disable}}" \
		orchestrators/compose infra/config docker-compose.yml \
		&& echo -e "$(_G)✓ yaml$(_0)"

test-lint-docker: ## Lint Dockerfiles — hadolint (errors fail, in Docker)
	@rc=0; for df in $$(git ls-files '*Dockerfile*' 2>/dev/null | grep -v node_modules); do \
		docker run --rm -i hadolint/hadolint hadolint --failure-threshold error - < "$$df" \
			|| { echo -e "$(_R)  $$df$(_0)"; rc=1; }; done; \
	[ $$rc -eq 0 ] && echo -e "$(_G)✓ dockerfiles$(_0)" || exit 1

test-lint-make: ## Lint Makefiles — checkmake (advisory, in Docker)
	@docker run --rm -v "$(CURDIR)":/d -w /d mrtazz/checkmake:latest Makefile 2>&1 | tail -20 || true; \
	echo -e "$(_G)✓ make (checkmake notes above are advisory)$(_0)"
...
test-scripts: ## Scripting: the phase smoke suite vs the live stack (auto-discovers kong's host port)
	@BASE_URL="$(_kong_base)" $(MAKE) --no-print-directory test-smoke

test-conformance: ## Engine conformance gate (m27) across the live engines
	@$(MAKE) --no-print-directory conformance

test-postman: ## Postman/newman: offer-capability proof + edge corpus vs the live stack (auto-discovers kong's host port)
